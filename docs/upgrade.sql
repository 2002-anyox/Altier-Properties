...
DO $mig_10$
BEGIN
  IF EXISTS (SELECT 1 FROM "drizzle"."__drizzle_migrations" WHERE hash = '22774247b29109bcea580528a19247d67bd31a433bac94e13e6bd06cc11dd394') THEN
    RAISE NOTICE 'already applied: 0010_signup';
  ELSE
    EXECUTE $mig_10_sql$
-- ---------------------------------------------------------------
...
ALTER TABLE "subscriptions"
  ...
  CHECK ("trial_ends_at" IS NULL OR "current_period_start" IS NULL
    ...
    $mig_10_sql$;
    INSERT INTO "drizzle"."__drizzle_migrations" (hash, created_at)
    VALUES ('22774247b29109bcea580528a19247d67bd31a433bac94e13e6bd06cc11dd394', 1787900700000);
    RAISE NOTICE 'applied: 0010_signup';
  END IF;
END $mig_10$;
...
DO $mig_11$
BEGIN
  IF EXISTS (SELECT 1 FROM "drizzle"."__drizzle_migrations" WHERE hash = 'e4821b8f24bf4ed2a4f1326ba9565d8535fb773e52110b2f18de2329f50a9bcc') THEN
    RAISE NOTICE 'already applied: 0011_settlement';
  ELSE
    EXECUTE $mig_11_sql$
...
ALTER TABLE "invoices" ADD CONSTRAINT "invoices_credit_has_booking"
    ...
    $mig_11_sql$;
    INSERT INTO "drizzle"."__drizzle_migrations" (hash, created_at)
    VALUES ('e4821b8f24bf4ed2a4f1326ba9565d8535fb773e52110b2f18de2329f50a9bcc', 1787900800000);
    RAISE NOTICE 'applied: 0011_settlement';
  END IF;
END $mig_11$;
...
DO $mig_12$
BEGIN
  IF EXISTS (SELECT 1 FROM "drizzle"."__drizzle_migrations" WHERE hash = 'd22e72910b3fb0f892849b3b72452a0d198e6b3c61bbb997004f9eff17508dd4') THEN
    RAISE NOTICE 'already applied: 0012_numbering_per_workspace';
  ELSE
    EXECUTE $mig_12_sql$
...
CREATE UNIQUE INDEX IF NOT EXISTS "maintenance_requests_reference_per_org"
    ...
    $mig_12_sql$;
    INSERT INTO "drizzle"."__drizzle_migrations" (hash, created_at)
    VALUES ('d22e72910b3fb0f892849b3b72452a0d198e6b3c61bbb997004f9eff17508dd4', 1787900900000);
    RAISE NOTICE 'applied: 0012_numbering_per_workspace';
  END IF;
END $mig_12$;
