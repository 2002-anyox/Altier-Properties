...
hold the permission. It is the same matrix the interface draws from, applied
where it protects something rather than only where it is visible.

**First run.** A new database has nobody in it. The sign-in page notices and
asks for a name, an email and a password, and the account it creates is the
owner. That window shuts permanently the moment any account has a password.
**Do it immediately after deploying**, before the address is shared: until you
...
one row the app cannot start without, the reminder settings. No people, no
properties, no charges. It refuses to run twice, and it records the migrations
...
DATABASE_URL='postgres://…' npm run db:seed   # development only: sample records
```

`db:seed` is for development. It truncates and replaces everything with the
Kampala sample portfolio, so never point it at a database holding real records.
A production deployment uses `db:init` or `docs/setup.sql` and starts empty. `docs/setup.sql` and `docs/upgrade.sql` are generated by `npm run db:sql`;
...
npm run db:init       # empty portfolio: reminder settings only
npm run db:seed       # development only: truncate and load samples
